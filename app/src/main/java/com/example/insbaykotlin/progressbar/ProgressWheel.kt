...
class ProgressWheel : View {
    ...
    private fun setupBounds(layout_width: Int, layout_height: Int) {
        ...
        if (!fillRadius) {
            ...
            circleBounds = RectF((paddingLeft + barWidth).toFloat(), (paddingTop + barWidth).toFloat(), (layout_width - paddingRight - barWidth).toFloat(),
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isSpinning) {
            ...
            if (!linearProgress) {
                val factor = 2.0f
                offset = (1.0f - Math.pow((1.0f - mProgress / 360.0f).toDouble(),
                        (2.0f * factor).toDouble())).toFloat() * 360.0f
                progress = (1.0f - Math.pow((1.0f - mProgress / 360.0f).toDouble(),
                        factor.toDouble())).toFloat() * 360.0f
            }
    ...
    fun setInstantProgress(progress: Float) {
        ...
        if (prog == mTargetProgress) {
            return
        }

        mTargetProgress = min(prog * 360.0f, 360.0f)
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        val ss = superState?.run {
            WheelSavedState(superState)
        }
